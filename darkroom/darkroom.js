...
function closeVideo() {
    ...
    const currentName = currentVideo.src.split("/")
        .pop();

    mp4Icons.forEach(icon => {
        const iconName = icon.video.split("/")
            .pop();
...
tutorialOverlay.style.background = "rgba(0,0,0,0.85)";
...
tutorialOverlay.style.zIndex = "99999";
tutorialOverlay.style.opacity = "0";
tutorialOverlay.style.transition = "opacity 0.2s";

const tutorialImg = document.createElement("img");
tutorialImg.src = "imgs/keys.png";
tutorialImg.style.maxWidth = "80%";
tutorialImg.style.maxHeight = "70%";
tutorialImg.style.borderRadius = "12px";
tutorialImg.style.boxShadow = "0 0 30px rgba(0,0,0,0.8)";

const text = document.createElement("p");
text.innerText = "Pressione X para fechar";
text.style.marginTop = "20px";
text.style.color = "#ccc";
text.style.fontFamily = "monospace";

tutorialOverlay.appendChild(tutorialImg);
tutorialOverlay.appendChild(text);
...
window.addEventListener("keydown", e => {
    if (e.key.toLowerCase() === "x" && tutorialOpen) {
        ...
        if (typeof cancelSound !== "undefined") {
            cancelSound.currentTime = 0;
            cancelSound.play()
                .catch(() => {});
        }
