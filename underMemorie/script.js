...
function startDragonAttack(){
  ...
  d.style.transition = "top 0.6s ease-out, opacity 0.6s ease-out";
  ...
  setTimeout(() => animateDragonOnce(d, ()=>fireBeamFromDragon(d)), 600);
  ...
  setTimeout(()=>dragons.forEach(d=>removeDragonWithBeam(d)), 2200);
}
...
function fireBeamFromDragon(dragon){
  ...
  const mouthY = dragonRect.top + dragonRect.height/2 - 50;
  beam.style.top = mouthY + "px";
  ...
  function updateBeam(){
    if(!dragon.parentNode){ beam.remove(); return; }
    const dragonRectNow = dragon.getBoundingClientRect();
    beam.style.top = dragonRectNow.top + dragonRectNow.height/2 - 50 + "px";
...
observer.observe(dialogBox, {childList:true, subtree:true, characterData:true});
